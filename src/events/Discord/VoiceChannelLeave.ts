import type { AnyVoiceChannel, Member } from "eris";
...
export default new Event("voiceChannelLeave", async (member: Member, channel: AnyVoiceChannel) => {
    const player = ruqa.audio.players.get(member.guild.id);
    if (!player) {
        return;
    }

    if (!member.bot && player.voiceChannelId === channel.id) {
        if (player.paused) {
            return;
        }
        player.pause(true);
